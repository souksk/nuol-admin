...
function RegistrationAdd({
  ...
  const [createRegistration, { data }] = useMutation(CREATE_REGISTRATION);
  ...
  const _registration = (values) => {
    ...
    createRegistration({ variables: paramQL }).then(async () => {
  ...
  const registeationAddValidation = Yup.object().shape({
    userId: Yup.string()
      .required('Required'),
  ...
  return (
    <div>
      <Modal
        show={showAddModal}
        onHide={_handleShowAddModalClose}
        size='lg'
      >
        ...
        <Modal.Body style={{ marginLeft: 50, marginRight: 50, padding: 50 }}>
          ...
          <Formik
            ...
            validationSchema={registeationAddValidation}
